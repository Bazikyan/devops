...
subnet_id=$(aws ec2 create-subnet --vpc-id="$vpc_id" --cidr-block='10.0.0.0/24' --region="$region" --query='Subnet.SubnetId')

route_table_id=$(aws ec2 create-route-table --vpc-id="$vpc_id" --region="$region" --query='RouteTable.RouteTableId')
